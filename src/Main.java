...
public class Main {
    ...
    static String sqlTrigger1 = "CREATE TRIGGER Insert_Sal" +
            ...
            " FOR EACH ROW" +
            " BEGIN" +
            " IF(NEW.salary<4000) AND (NEW.job='教授') THEN " +
            " INSERT INTO sallog(name,job,old,new) " +
            " VALUES(NEW.name,NEW.job,NEW.salary,4000);" +
            " SET NEW.salary=4000;" +
            " END IF;" +
            " END;";
    ...
    public static boolean insert(Teacher teacher) throws SQLException {
        ...
        pst.setString(2,teacher.getJob());
        pst.setInt(3,teacher.getSalary());
    ...
    public static ArrayList<Teacher> search(String sql) throws SQLException {
        ...
        while(rs.next()) list.add(new Teacher(rs.getString(1),rs.getString(2),rs.getInt(3)));
        return list;
    }
    //设置触发器
        ...
        while(flag){
            switch (input.nextInt()){
                ...
                case 3:
                    ...
                    for(int i=0;i<list.size();++i)System.out.println(list.get(i).getName()+" "+list.get(i).getJob()+" "+list.get(i).getSalary());
